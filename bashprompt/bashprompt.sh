#!/bin/bash
# Original author: Dave Vehrs 
# Maintainer: Knut Eirik Leira Hjelle

# Current Format: USER@HOST [dynamic section] { CURRENT DIRECTORY } <GIT branch if present> $ 
...
#   Lightgray     == Current user does have write priviledges
# NOTE:
...
function promptcmd () {
    ...
    local LOCALHOSTNAME=$(scutil --get LocalHostName)
    if [[ ${TTY} ]]; then 
    ...
    case ${TERM} in 
        xterm*  )  
            local TITLEBAR="\[\033]0;\u@${LOCALHOSTNAME}: { \w }  \007\]"
            ;;
        *       )  
            local TITLEBAR=''                               
            ;;
    ...
    if [ ${SSH_FLAG} -eq 1 ]; then 
       PS1="${PS1}\[${COLOR_CYAN}\]\h "
        ...
        PS1="${PS1}\[${COLOR_LIGHTBLUE}\]\h "
        else 
            local parent_process=`ps -p ${PPID} -c -o command | tail -1`
    ...
    # Working directory
    if [ -w "${PWD}" ]; then 
        PS1="${PS1}\[${COLOR_LIGHTGRAY}\]$(prompt_workingdir)"
    else                                 
...
function load_prompt () {
    # Get PIDs
    local parent_process=$(ps -p ${PPID} -c -o command | tail -1)
    local my_process=$(ps -p $$ -c -o command | tail -1)
